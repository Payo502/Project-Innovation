...
namespace UtilityPackage.Utility.UnityFunctionHandlers
{
    ...
    {
        [SerializeField] private UnityFunction reactTo;

        protected virtual void Start()
        {
            if ((reactTo & UnityFunction.Start) == 0) return;
        ...
        {
            if ((reactTo & UnityFunction.OnEnable) == 0) return;
        ...
        {
            if ((reactTo & UnityFunction.OnDisable) == 0) return;
        ...
        {
            if ((reactTo & UnityFunction.OnDestroy) == 0) return;

            ReactToEvent(UnityFunction.OnDestroy);
        }

        /// <summary>
        ///     Will be called by the <see cref="AbstractFunctionHandler" /> as a reaction to given
        ///     <see cref="Enums.UnityFunction" />s in the inspector
        /// </summary>
